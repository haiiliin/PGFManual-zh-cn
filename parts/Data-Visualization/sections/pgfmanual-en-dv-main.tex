...
\begin{datavisualizationoperation}{data group}{\opt{\oarg{选项}}\marg{名字}\opt{|+=|\marg{数据指定}}} % \begin{datavisualizationoperation}{data group}{\opt{\oarg{options}}\marg{name}\opt{|+=|\marg{data specifications}}}
    ...
    \begin{itemize}
        ...
        \item |data group| \opt{\oarg{选项}} \marg{名字} |=| \marg{数据指定}
        % \item |data group| \opt{\oarg{options}} \marg{name} |+=| \marg{data specifications}
        \item |data group| \opt{\oarg{选项}} \marg{名字} |+=| \marg{数据指定}
        % \item |data group| \opt{\oarg{options}} \marg{name}
        \item |data group| \opt{\oarg{选项}} \marg{名字}
    \end{itemize}
    %
    ...
    %
    在第一种情况下，创建了一个名为 \meta{名字} 的新数据组（将擦除同名的现有数据组），并将以下 \meta{数据指定} 存储在该数据组中。数据组将不会被馈送到渲染管道，但此时它就像被解析一样被解析。所定义的数据组是全局定义的，因此可以在后续可视化中使用它。\meta{选项} 与解析的 \meta{数据指定} 一起保存。
    ...
    在第三种情况下（注意到 \meta{名字} 后面既没有等号也没有加号），插入先前定义的名为 \meta{名字} 的数据组的内容。还将执行 \meta{选项}。
