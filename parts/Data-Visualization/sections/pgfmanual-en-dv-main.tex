...
\begin{command}{\datavisualization\opt{\oarg{数据可视化选项}}\meta{数据说明}|;|} %\begin{command}{\datavisualization\opt{\oarg{data visualization options}}\meta{data specification}|;|}
    ...
    根据这些选项，\meta{数据说明} 用于提供实际的要可视化的数据。该语法有点类似于 |\path| 这样的命令：\meta{数据说明} 后跟本地选项和参数的关键字序列，以分号结尾。（实际上，就像 |\path| 命令一样，不需要在开始时指定 \meta{数据可视化器选项}，但是在 \meta{数据说明} 内的任何位置都可以提供方括号包围的附加选项）。

    % The different possible keywords inside the \meta{data specification} are explained in the following.

    下面解释了 \meta{数据说明} 中可能存在的不同关键字。
\end{command}
...
\begin{datavisualizationoperation}{data group}{\opt{\oarg{选项}}\marg{数据组名称}\opt{|+=|\marg{数据说明}}} % \begin{datavisualizationoperation}{data group}{\opt{\oarg{options}}\marg{name}\opt{|+=|\marg{data specifications}}}
    ...
    您可以将整个 \meta{数据说明} 存储在一个\emph{数据组}中。这允许您在多个地方重用数据，而不必将数据写入外部文件。
    ...
    \begin{itemize}
        ...
        \item |data group| \opt{\oarg{选项}} \marg{数据组名称} |=| \marg{数据说明}
        % \item |data group| \opt{\oarg{options}} \marg{name} |+=| \marg{data specifications}
        \item |data group| \opt{\oarg{选项}} \marg{数据组名称} |+=| \marg{数据说明}
    ...
    %
    在第一种情况下，创建了一个名为 \meta{数据组名称} 的新数据组（将擦除同名的现有数据组），并将以下 \meta{数据说明} 存储在该数据组中。数据组将不会被馈送到渲染管道，但此时它就像被解析一样被解析。所定义的数据组是全局定义的，因此可以在后续可视化中使用它。\meta{选项} 与解析的 \meta{数据说明} 一起保存。

    % In the second case, an already existing data group is extended by adding the \meta{data specifications} to it.

    在第二种情况下，通过向现有数据组添加 \meta{数据说明} 来扩展该数据组。
...
\begin{datavisualizationoperation}{scope}{\opt{\oarg{选项}}\marg{数据说明}} % \begin{datavisualizationoperation}{scope}{\opt{\oarg{options}}\marg{data specification}}
    ...
    分组可用于嵌套分层数据集。\meta{选项} 将以 |/pgf/data| 路径执行，且仅适用于 \meta{数据说明} 中指定的数据集，其中可能再次包含 |data| 或 |scope| 命令：
    %
\begin{codeexample}[code only]
\datavisualization...
